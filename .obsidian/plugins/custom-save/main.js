...
var createNotice = (message, duration) => new import_obsidian.Notice(`Custom Save: ${message}`, duration);
...
// src/main.ts
var isMarkdownFile = (file) => file instanceof import_obsidian2.TFile && file.extension === "md";
var CustomSavePlugin = class extends import_obsidian2.Plugin {
  constructor() {
    super(...arguments);
    this.runSaveCommand = async (checking, editor, ctx) => {
      var _a, _b;
      if (!ctx.file)
        return;
      if (checking) {
      ...
      await ((_a = this.app.workspace.getActiveFileView()) == null ? void 0 : _a.save());
      ...
      await ((_b = this.app.workspace.getActiveFileView()) == null ? void 0 : _b.save());
    };
  }
  async onload() {
...
var CustomSaveSettingTab = class extends import_obsidian2.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    this.settingItemMap = /* @__PURE__ */ new Map();
    this.plugin = plugin;
  }
  display() {
    ...
    let currentValue = "";
    new import_obsidian2.Setting(containerEl).setName("Add command").addDropdown((dropdown) => {
      dropdown.addOptions(
        this.plugin.app.commands.listCommands().filter(
          (command) => !setting.commandIds.includes(command.id) && (command.editorCallback || command.editorCheckCallback) && command.id !== "custom-save:save"
        ).map((command) => {
          return {
            label: command.name,
            value: command.id
      ...
      button.setButtonText("Add").onClick(() => {
        if (!currentValue)
          return;
        this.plugin.settingManager.updateSettings((setting2) => {
          setting2.value.commandIds.push(currentValue);
        });
        containerEl.empty();
        this.display();
      });
    ...
    setting.commandIds.forEach((commandId) => {
      ...
      const setting2 = new import_obsidian2.Setting(containerEl).setName(
        `${(_a = command == null ? void 0 : command.name) != null ? _a : commandId} ${!command ? "(not found)" : ""}`
      ).setDesc((_b = command == null ? void 0 : command.id) != null ? _b : "").addButton((button) => {
        button.setButtonText("Remove").onClick(() => {
          this.plugin.settingManager.updateSettings((setting3) => {
            setting3.value.commandIds = setting3.value.commandIds.filter(
              (id) => id !== commandId
            );
          });
          containerEl.empty();
      ...
      if (!command)
        setting2.nameEl.addClass("custom-save-error");
      this.settingItemMap.set(commandId, setting2);
    });
  }
};
